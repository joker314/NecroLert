...
function getDate(el){
  console.log(el);
  var d = el.innerHTML;
  var d = d.replace("Today", new Date().getFullYear() + "-" + (new Date().getMonth() + 1) + "-" + new Date().getDate());
  var d = d.replace("Yesterday", new Date(Date.now() - 86400000).getFullYear() +
    "-" + (new Date(Date.now() - 86400000).getMonth() + 1) + 
...
for (var i = 1; i < postheaders.length; i++){
 if (getDate(postheaders[i].children[1]).valueOf() - getDate(postheaders[i - 1].children[1]).valueOf() > 5097600000){
   postheaders[i].style.backgroundColor = "red";
 }
}

if(new Date().valueOf() - getDate(postheaders[postheaders.length - 1].children[1]).valueOf()   > 5097600000){
